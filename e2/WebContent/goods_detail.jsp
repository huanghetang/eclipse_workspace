...
//第一次页面加载完毕加载评论
/*  $(function(){
	 appendCom('${product.pid}');
 }); */

  //加载评论
 function appendCom(pageNum){
	 	//安全检查
	 	if(pageNum<1){
	 		pageNum=1;
	 	}
	 	if(pageNum>'${pageBean.end}'){
	 		pageNum = '${pageBean.end}';
	 	}
		//清空用户评论
		$("#comdiv").html("");
		//清空分页条
		...
		//后台获取分页数据
	  $.get(
		"ProductController?method=findCommentPageBean",	  
	  	{"pid":'${product.pid}',"pageNum":pageNum},
	  	function(data){
	  		//获取分页中的评论list
	  		var com = data.data;
	  		...
	  		//安全检查
	  		if(!com){//pageBean.data(分页中的属性)
	  			return null;
	  		}
	  		//遍历数据集合
	  		$.each(com,function(i,n){
	  			//追加评论
	  			 $("#comdiv").append("<span>"+n.nickname+":"+n.comment+"&nbsp;&nbsp;<img src='themes/ecmoban_jumei/images/stars"+n.level+".gif'/></span>"
	  			 		+'<span style="float:right;">'+n.createTime+'</span><br/>');
	  			
	  		});
	  		//追加分页条
  			$("#pager").html(" 总计 "+total+"个记录，共"+end+" 页,当前第"+pageNum+"页 <span> "+
					
					"<a href='javascript:;' onclick='getPageCom(1)'>第一页</a>"+
					
						"<a href='javascript:;' onclick='getPageCom("+(pageNum-1)+")'>上一页</a>"+
					
					
					
						"<a href='javascript:;' onclick='getPageCom("+(pageNum+1)+")'>下一页</a>"+
				
						"<a href='javascript:;' onclick='getPageCom("+end+")'>最末页</a>"+
					"</span>"); 
	  	}
	  );
 } 
...
<div class="AreaR" style="width:950px;">
	...
	<div class="blank"></div>
		<div class="box_1">
				...
				<div class="boxCenterList clearfix" style="height:1%;" id="comdiv">
 				<c:if test="${not empty pageBean.data}" >
						<c:forEach items="${pageBean.data}" var="com">
							...
							<c:if test="${not empty com.createTime}">
								<span style="float: right;">${com.createTime}</span>
							</c:if>
							 <br/>
						</c:forEach>	
				</c:if> 
					...
					<div id="pagebar" class="f_r">
							<div id="pager">
								...
								<c:if test="${pageBean.pageNum >2}">
									<a href="javascript:;" onclick="getPageCom('${pageBean.pageNum-1}')">上一页</a> 
								</c:if>
								
								<c:if test="${pageBean.pageNum < (pageBean.end-1)}">
									<a href="javascript:;" onclick="getPageCom('${pageBean.pageNum+1}')">下一页</a>
								</c:if>
									<a href="javascript:;" onclick="getPageCom('${pageBean.end}')">最末页</a>
								</span>
							</div>
					</div>
					<div class="blank5"></div>
