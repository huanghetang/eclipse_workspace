...
function reg(str){
  var bt=_$(str+"_b").getElementsByTagName("h2");
  for(var i=0;i<bt.length;i++){
    bt[i].subj=str;
    bt[i].pai=i;
    bt[i].style.cursor="pointer";
    bt[i].onclick=function(){
      _$(this.subj+"_v").innerHTML=_$(this.subj+"_h").getElementsByTagName("blockquote")[this.pai].innerHTML;
      for(var j=0;j<_$(this.subj+"_b").getElementsByTagName("h2").length;j++){
        var _bt=_$(this.subj+"_b").getElementsByTagName("h2")[j];
        var ison=j==this.pai;
        _bt.className=(ison?"":"h2bg");
      }
    }
  }
  _$(str+"_h").className="none";
  _$(str+"_v").innerHTML=_$(str+"_h").getElementsByTagName("blockquote")[0].innerHTML;
}
