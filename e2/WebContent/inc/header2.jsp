...
<!-- 						<script type="text/javascript" src="js/transport.js"></script>
						...
						<font id="ECS_MEMBERZONE"> 欢迎${user.nickname }光临本店
							| <a href="${pageContext.request.contextPath}/RegisterServlet?method=logout">退出</a>
						</font>
						</c:if>
					</div>
					<ul class="top_bav_l">
					...
					<div class="header_r">
						<a href="${path }/OrderController?method=listOrder">我的订单</a>
						<a href="${path }/ProductController?method=solrList">商品列表</a>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="header_top logo_wrap clearfix">
			...
			<div class="ser_n">
				<form id="searchForm" class="searchBox clearfix" name="searchForm"
					 method="post" action="${path }/ProductController?method=solrList">
					<span class="ipt1">
						<input name="keywords"
								placeholder="请输入您想购买的商品" type="text" id="keyword" value="${keywords}"
								class="searchKey" /></span> <span class="ipt2">
			...
			<ul class="cart_info">
				<li id="ECS_CARTINFO">
					<div class="top_cart">
						...
						<a href="${pageContext.request.contextPath }/CartController?method=list&flag=true" class="shopborder">去购物车结算</a>
					</div>
				</li>
			</ul>
		</div>
...
<div class="menu_box clearfix">
	...
	<div class="block" style="width:<%=width%>">
		<div class="menu"  id="menu1">
			<a id="menu_index" href="${path }/RegisterServlet?method=index" class="<%=className%>">首页</a> <a
...
<script>
    $(function () {
    	var lis="";
    	$.ajax({
    		url:"${pageContext.request.contextPath}/index?method=imgList",
    		async:false,
    		type:"post",
    		success:function(data){
    			$.each(data,function(i,n){
					lis+="<a href='#'><img src="+n.image+"></a>"; 
				});
				$(".shutter-img").append(lis); 
    		},
    		dataType:"json"
    	}); 
