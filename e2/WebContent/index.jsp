...
<!-- 	<div class="container" id="idTransformView">
	...
	<div class="block clearfix Main">
				...
				<tr>
					<td>
						<a href='javascript:;'>
							...
							<!-- <embed id="advideo" src="" width="240" height="140" autostart="true" loop="false"/> -->
		...
		<c:if test="${not empty requestScope.fproduct}">
			<div class="xm-box">
				<h4 class="title">
					<span>我的足迹</span>  <a class="more" href="javascript:;">更多</a>
				</h4>
				<div id="show_new_area" class="clearfix">
				<c:forEach items="${requestScope.fproduct}" var="f">
					<div class="goodsItem">
						<a href="${path}/ProductController?method=productDetail&pid=${f.pid}"><img
							src="${f.pimage }"
							alt="${f.pname }" class="goodsimg" /></a><br />
						<p class="f1">
							<a href="${path}/ProductController?method=productDetail&pid=${f.pid}" title="${f.pname }">${f.pname }</a>
						</p>
						市场价：<font class="market">${f.market_price}元</font> <br /> 本店价：<font class="f1">
							${f.shop_price}元 </font>
					</div>
			</c:forEach>
				</div>
...
<script>
	$(function(){
		var lis="";
		$.ajax({
			url:"${pageContext.request.contextPath}/index?method=adList",
			async:true,
			type:"post",
			...
			/* 		if(i==0){
						//视频
						advideo.src = n.ad_image;
					} */
					/*  lis+="<li><a href='${pageContext.request.contextPath}/index?method=adInfo&id="+n.ad_id+"'>"+n.ad_title+"</a></li>";  */
					/* lis+="<li><a href='javascript:;' onclick='optionAd("+n.ad_id+")'>"+n.ad_title+"</a></li>"; */
					lis+="<li><a href='${pageContext.request.contextPath}/index?method=adInfo&id="+n.ad_id+"' target='_blank'>"+n.ad_title+"</a></li>";
				})
...
/* 	function optionAd(id){
		alert('111'+id);
		window.open
		location.href="${pageContext.request.contextPath}/index?method=adInfo&id="+id;
	} */
</script>
</html>